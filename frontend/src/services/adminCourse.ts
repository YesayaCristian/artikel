...
export async function fetchCourses(): Promise<{ mata_kuliah: ApiCourse[] }> {
  return http<{ mata_kuliah: ApiCourse[] }>("/api/mata-kuliah/list/");
}

export async function getAdminCourse(id: number): Promise<ApiCourse> {
  return http<ApiCourse>(`/api/mata-kuliah/${id}/`);
}
...
export async function createAdminCourse(data: {
  ...
  return http("/api/mata-kuliah/create/", {
    method: "POST",
...
export async function updateAdminCourse(
  ...
  return http(`/api/mata-kuliah/${id}/update/`, {
    method: "POST",
    body: JSON.stringify(data),
  });
}
...
export async function deleteAdminCourse(id: number) {
  return http(`/api/mata-kuliah/${id}/delete/`, { method: "DELETE" });
}
